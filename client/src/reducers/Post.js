...
export default function(state= initialState, action) {
    ...
    switch(type) {
        ...
        case ADD_POST:
            return {...state,posts:[payload, ...state.posts],loading:false}// add post to array posts=new+old post  
        case DELETE_POST:
            return {...state,posts:state.posts.filter(post => post._id !== payload),loading:false}
        case POST_ERROR:
            return {...state,error:payload,loading:false}
        case UPDATE_LIKES:
            return {...state,posts:state.posts.map(post => post._id === payload.id 
        ...
        case REMOVE_COMMENT:
            return {...state,post:{...state.post,
                comments:state.post.comments.filter(comment =>comment._id !== payload)},loading:false}
        default:
            return state
    }
}